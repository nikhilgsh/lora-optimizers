...
It is natural to ask whether similar gains can carry over to LoRA. We found, however, that applying Muon to each factor does not improve over Adam. Part of the problem is structural: the model sees the \emph{product} $\mat{B}\mat{A}$, while the optimizer acts on the \emph{factors} $\mat{A}$ and $\mat{B}$ individually. Several concurrent works~\cite{imuon,loramuon,tilde_csd} correct this with variants of a product-aware Muon step we refer to as \emph{Product Muon} (\Cref{sec:product-muon}).
...
The rest of the paper proceeds as follows. In \Cref{sec:muon} we
first give a new interpretation of Muon as a descent method that controls the change in the per-sample loss.
We then use this viewpoint to motivate a preconditioned Muon in \Cref{sec:preconditioned-muon}. In \Cref{sec:product-muon} we show how to make a Muon update product-aware for LoRA, resulting in Product Muon. 
We finally arrive at \methodname{} by combining preconditioned Muon, product aware Muon and a magnitude balancing rule in~\Cref{sec:method}. This magnitude balancing is a separate rescaling that guarantees that each low rank factor has the same update magnitude in spectral norm, while also controlling the magnitude of the merged-weight updates. All proofs are in \Cref{app:proofs}.
...
In \Cref{sec:exp-ablation} we separate the roles of curvature and magnitude.
...
\section{A new loss sensitivity \texorpdfstring{\rev{viewpoint}}{viewpoint} on Muon}\label{sec:muon}
Our first step towards deriving  \methodname{} is to re-interpret Muon as a descent method that controls the change in the per-sample loss. 
For this section we will temporarily ignore the product structure of LoRA~\eqref{eq:lora} and consider a single linear layer $\mat{W} \in \R^{d_{\mathrm{out}} \times d_{\mathrm{in}}}$ with loss $\cL(\mat{W})$ and gradient $\mat{G} = \nabla_W \mathcal{L}(\mat{W})$.


Muon is based on a spectral LMO~\cite{bernstein2025modular,pmlr-v38-carlson15,carlson2015preconditioned} where in the LMO minimizes the local linearization of the loss subject to a ball constraint,
\begin{equation}
 \begin{array}{ll}
...
and $\mat{G}=\mat{U}\mat{\Sigma} \mat{V}^\top$ is the reduced SVD (see~\Cref{lem:muon-lmo}). Though 
...
% Next we show how to adapt this LMO to be aware of the product structure in LoRA~\eqref{eq:lora}.


\subsection{Per-Sample Loss Control}\label{sec:sample-loss-lmo}

Here we show a new viewpoint for Muon. To describe this viewpoint, we first introduce some notation.
  Let $x\sim \cD$ be the distribution of inputs to the layer, thus for an input $x\in \R^{d_{\mathrm{in}}}$ the output is $\mat{W}x\in \R^{d_{\mathrm{out}}}$. The distribution $\cD$ is induced by the distribution of the data and by the parameters of the \chg{preceeding}{preceding} layers,  but we will omit this dependence for  simplicity.
Let 
$\cL(\mat{W}) =\EE{x\sim \cD}{ \ell(\mat{W}x)}$ be the loss, where $\ell(\mat{W}x)$ is the loss on a single input $x$. 
% We denote the gradient of the loss with respect to $\mat{W}$ as $\mat{G} = \nabla \cL(\mat{W})$.
...
Here we instead motivate Muon through a \emph{loss} sensitivity bound. 

For this viewpoint we will suppose the gradient is $L>0$ bounded for every input:
\begin{equation} \label{eq:Lipschitz}
    \| \nabla_W \ell(\mat{W}x) \|_F \leq L, \quad \mbox{for every } x \in \support(\cD).
\end{equation} 
...
\]
where $\mat{G}(x) = \nabla_W \ell(\mat{W}x)$ is the per-input gradient. 
Imposing the linearized per-sample bounds~\eqref{eq:loss-diff} in place of the spectral-norm constraint within~\eqref{eq:lmo} gives
\begin{equation}
 \begin{array}{lll}
  \underset{\Delta\mat{W}}{\mathrm{minimize}} &
...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since we lack access to the distribution on inputs $\cD$.
So instead, we will enforce it indirectly, by defining an outer-gradient set $\mathcal{G}_L$ that contains all possible per-input gradients $\mat{G}(x)$ (and negative gradients $-\mat{G}(x)$), so that
\begin{equation} \label{eq:outgrad1}
    \max_{x \in \support(\cD)}  |\ip{\mat{G}(x), \Delta\mat{W}}|
    \leq   \max_{\widetilde{\mat{G}} \in \mathcal{G}_L} \; \ip{\widetilde{\mat{G}}, \Delta\mat{W}}.
\end{equation}
Thus imposing that the right-hand side of \eqref{eq:outgrad1}
is less than $\tau$ is a sufficient condition for the constraint in~\eqref{eq:sample-lmo} to hold.
To build $\mathcal{G}_L$ we collect some  properties of the $\mat{G}(x)$ gradients in the following lemma.
\begin{restatable}[Properties of gradients]{lemma}{gradprop} 
  \label{lem:gradprop} Suppose the gradient is $L$ bounded according to~\eqref{eq:Lipschitz}. 
    Let $g(x)=\vec(\mat{G}(x)) \in \R^{d_{\mathrm{out}} d_{\mathrm{in}}}$ for $x \in \support(\cD)$.  The gradients satisfy the following properties:
    \begin{enumerate}
        \item {\bf Rank one:} $\mat{G}(x) = b(x) x^\top$  for some $x \in \R^{d_{\mathrm{in}}}$ and $b(x) \in \R^{d_{\mathrm{out}}}$
        \item {\bf Bounded product:} $\| g(x)\|_2 = \|b(x)\|_2  \|x\|_2\leq L. $
        \end{enumerate}
\end{restatable}
...
\begin{equation} 
\label{eq:outgrad} 
\mathcal{G}_L := \left \{ \widetilde{\mat{G}} = b x^\top ~\bigg |~  
b\in\mathbb{R}^{d_{\mathrm{out}}},
    \ x\in\mathbb{R}^{d_{\mathrm{in}}}, \
\|b\|_2\|x\|_2 \leq L \right \}.
\end{equation}
Since $\mat{G}(x) \in \mathcal{G}_L$ by~\Cref{lem:gradprop}, we can use this $\mathcal{G}_L$ in our upper bound~\eqref{eq:outgrad1}.

Next we show that Muon can be seen as \chg{a}{an} LMO method such as~\eqref{eq:sample-lmo}, but where gradients are taken from the outer-gradient set.
  \begin{restatable}[Loss bounded View of Muon]{lemma}{muonview}  \label{lem:muon}
The closed form to the steepest descent method
\begin{equation}\label{eq:muon-alt}
    \min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}} \quad \mbox{subject to} \quad
|\langle \hat{\mat{G}},\Delta\mat{W} \rangle| \le \tau \qquad \text{for  every }\quad  \hat{\mat{G}} \in \mathcal{G}_L,
\end{equation}
is given by the Muon update
\[ \Delta\mat{W} =  - \eta  \,\mat{U} \mat{V}^\top,\]
where $\mat{U}  D \mat{V}^\top = \mat{G}$  is the reduced SVD and $ \eta:= \frac{\tau}{L} $ is the step size.
\end{restatable}

What this lemma tells us is that  the Muon update guarantees that the linearized sensitivity constraint remains bounded. \chg{Specfically}{Specifically}, if we use Muon with a learning rate $\eta$ and our gradients are $L$ bounded, then due to~\eqref{eq:outgrad1} we know that 
\[ |\ip{\mat{G}(x), \Delta\mat{W}} | \leq \eta L, \quad \mbox{for every} \quad x \in \support(\cD). \]
If either the step size $\eta$ or the Lipschitz constant is too big, then we have little control over the oscillations of the function. 
% In particular if the step size is too big, then the local linearization of the loss becomes a poor proxy for the loss.
% Finally, 
Yet a further issue is that this view relies on the assumption that the gradient is $L$ bounded~\eqref{eq:Lipschitz}, but this does not always hold in practice or may be effectively infinite. In particular, even for simple linear regression problems these gradients can be unbounded. 

\subsection{Preconditioned Muon}\label{sec:preconditioned-muon}

Our derivation of Muon in~\Cref{lem:muon} relied on assuming the gradients are bounded in the standard Frobenius norm~\eqref{eq:Lipschitz}. Though this is a common assumption, it does not always hold, or it may give a loose bound. 
Even for functions for which the gradients are not bounded in the standard Frobenius norm, they may be bounded in a \emph{local} metric such as
\begin{equation} \label{eq:Lipschitz-local}
    \| \mat{G}(x) \|_{\mat{\Sigma}} \leq L, \quad \mbox{for every } x \in \support(\cD),
\end{equation}
where $\mat{G}(x) = \nabla_W \ell(\mat{W}x)$ and
\begin{align}
    \| \mat{G}(x) \|_{\mat{\Sigma}}^2 
    &:= \vec(\mat{G}(x))^\top \mat{\Sigma}^\dagger \vec(\mat{G}(x)),  \label{eq:leverage-score} \; \mbox{where} \;
    \mat{\Sigma} \;:=\EE{x}{\vec(\mat{G}(x))\vec(\mat{G}(x))^\top} \in \R^{nm \times nm}.
\end{align} 

The scalars $\| \mat{G}(x) \|_{\mat{\Sigma}}$ in~\eqref{eq:Lipschitz-local} are often referred to as leverage scores, which are well studied quantities for which  there are many known bounds from approximation theory and randomized linear algebra~\cite{CohenDavenportLeviatan2013,CohenMigliorati2017,RauhutWard2012}. We will refer 
% to~\eqref{eq:Lipschitz-local} as the local gradient bound, since it depends on a local metric defined by $\mat{\Sigma},$ and 
the scalars $\| \mat{G}(x) \|_{\mat{\Sigma}}$  as the gradient leverage scores.

\begin{cut}
The gradients leverage scores are bounded in many setting where the gradients are unbounded in the Frobenius norm. In~\Cref{asec:grad-leverage} we discuss several examples of this, including linear regression~\Cref{lem:leverage-bound-sphere} where gradient leverage scores are bounded and constant but the Frobenius norm of the gradients is unbounded. We also show that gradient leverage scores are bounded by a constant when the set of inputs are finite (regardless of $\mat{W}$) (\Cref{lem:finite-support}), and  in expectation for any distribution~(\Cref{lem:expectation}).
\end{cut}
\rev{[The Markov-inequality argument replaces this paragraph.]}

In conclusion, assuming locally bounded gradients~\eqref{eq:Lipschitz-local} can often be a more reasonable assumption than assuming globally bounded gradients~\eqref{eq:Lipschitz}. 
Which in turn suggests we should use~\eqref{eq:Lipschitz-local} in place of~\eqref{eq:Lipschitz} to define an outer-gradient set given by 
\begin{equation} 
\label{eq:outgrad-precon} 
\mathcal{G}_L(\mat{\Sigma}) := \left \{ \widetilde{\mat{G}} = b x^\top ~\bigg |~  
b\in\mathbb{R}^{d_{\mathrm{out}}},
    \ x\in\mathbb{R}^{d_{\mathrm{in}}}, \
\ip{\mat{\Sigma}^\dagger \vec(\widetilde{\mat{G}}), \vec(\widetilde{\mat{G}})} \leq L \right \}.
\end{equation}
Though this is a mathematically sound approach, using $\mathcal{G}_L(\mat{\Sigma})$ within an LMO approach such as~\eqref{eq:muon-alt} leads to an impractical algorithm that relies on storing and computing inverse square roots of $\mat{\Sigma}$ which is a large $nm \times nm$ matrix. To arrive at a practical algorithm, \chg{will we}{we will} use a Kronecker approximation of $\mat{\Sigma}$.



\paragraph{Kronecker approximation.}
Following~\cite{shampoo,kfac}, we approximate $\mat{\Sigma}$ by a Kronecker product of symmetric positive definite matrices $\mat{P}\in \R^{d_{\mathrm{out}} \times d_{\mathrm{out}}}$ and $\mat{Q} \in\R^{d_{\mathrm{in}} \times d_{\mathrm{in}}}$,
\begin{equation} \label{eq:pq-factor}
     \mat{\Sigma}  \approx \mat{Q} \otimes \mat{P}.
\end{equation}
We treat the \emph{preconditioners} $\mat{P}$ and $\mat{Q}$ as given for now, deferring their update rule to \Cref{app:matnormal}.


Using the factorization $\mat{\Sigma}  \approx \mat{Q} \otimes \mat{P}$ above in the outergradient set~\eqref{eq:outgrad-precon}  and using this outergradient set in~\eqref{eq:sample-lmo} gives a preconditioned version of Muon.
...
\begin{equation}\label{eq:precond-muon-super}
    \min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}} \quad \mbox{subject to} \quad |\ip{\hat{\mat{G}},\Delta\mat{W}}| \le \tau \qquad \text{for  every } \quad\hat{\mat{G}} \in \mathcal{G}_L(\mat{Q} \otimes \mat{P}),
\end{equation}
is equivalent to
\begin{equation}\label{eq:precond-muon-constraint}
\min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}} \quad \mbox{subject to} \quad \left\| \mat{P}^{1/2} \Delta\mat{W} \mat{Q}^{1/2} \right\|_2 \le \frac{\tau}{\sqrt L}.
\end{equation}
To which a closed form is given by
\begin{equation} \label{eq:polorafull-update}
 \Delta\mat{W} = -\frac{\tau}{\sqrt L} \mat{P}^{-1/2} \msign(\mat{P}^{-1/2} \mat{G} \mat{Q}^{-1/2}) \mat{Q}^{-1/2}.
\end{equation}
\end{restatable}
In the next two sections we will show how to adapt the Muon and preconditioned Muon ~\eqref{eq:polorafull-update} to be aware of the product structure in LoRA~\eqref{eq:lora}, which will yield \methodname{}.
...
Here we combine the ideas of preconditioning and product-awareness to derive \methodname{}. We first derive the update direction, and then set the magnitude of the factor updates separately.


\paragraph{Direction.}
So far we have described an update for the weight matrix $\mat{W} \in \R^{d_{\mathrm{out}} \times d_{\mathrm{in}}}$. We now adapt it to LoRA~\eqref{eq:lora}, deriving the \emph{direction} first and then setting the \emph{magnitude} of the factor updates. Replacing $\Delta\mat{W}$ in \eqref{eq:precond-muon-constraint} with its linearized product update~\eqref{eq:DeltaW-lin} gives the LMO
\begin{equation}
    \begin{array}{ll}
...
\end{equation}
Since substituting~\eqref{eq:collapse-constraint} into~\eqref{eq:decoupled-direction-lmo} makes each factorwise LMO an instance of \Cref{lem:precond-muon} with $L=1$, the two minimizers are $\Delta\mat{A} = -\tau \mat{D}_A$ and $\Delta\mat{B} = -\tau \mat{D}_B$, where
\begin{equation}
  \begin{aligned}
  \mat{D}_A &=
  \mat{C}_B^{-1/2}\,
...
\begin{itemize}[leftmargin=1.7em,itemsep=1pt,topsep=2pt]
  \item \textbf{Product-awareness}. The direction LMO~\eqref{eq:decoupled-lora-lmo} uses the structure of the merged product $\mat{B}\mat{A}$ as opposed to treating $\mat{A}$ and $\mat{B}$ as independent matrices.
...
The bounded-product property then follows from~\eqref{eq:Lipschitz}, since
\[
\norm{g(x)}_2=\norm{\mat{G}(x)}_F=\norm{b(x)x^\top}_F=\norm{b(x)}_2\norm{x}_2\le L .
\]
\end{proof}
...
Let $\tau\ge 0$, let $\mat{G}(x)$ be the per-input gradients in~\eqref{eq:sample-lmo}, and let $\mat{G}=\EE{x\sim\cD}{\mat{G}(x)}$. Then the per-sample LMO~\eqref{eq:sample-lmo} attains a finite minimum.
\end{proposition}
\begin{proof}
The feasible set is nonempty, closed, and convex, since it is an intersection of closed slabs and contains $\Delta\mat{W}=0$. For every feasible $\Delta\mat{W}$, by \rev{Jensen's inequality} \del{Jensen's} we have that 
\[
...
\[
    ...
    -\eta\norm{\mat{G}}_{\mathrm{nuc}}.
\]
Let $\mat{G}=\mat{U}\mat{\Sigma} \mat{V}^\top$ be a reduced singular value decomposition, so
$\mat{\Sigma}=\diag(\sigma_i(\mat{G}))$. The choice
\[
...
Next we give the proof for the alternative characterization of Muon.
%%%%%%%%%%%%%%%%%%%%
\muonview*
%%%%%%%%%%%%%%%%%%%%
\begin{proof}
  First note that given $\hat{\mat{G}} = b x^\top$ we have that
  \[ |\ip{\hat{\mat{G}},\Delta\mat{W}}| \leq \tau \Leftrightarrow |b^\top \Delta\mat{W} x| \le \tau.\]
Consequently
  the constraints in~\eqref{eq:muon-alt} can be written as
  \begin{equation} \label{eq:ernaoidnwlaiwnd}
       \max_{x \in \R^{m}, b \in \R^n\;:\;   \|x\|_2\|b\|_2 \leq L}   |b^\top \Delta\mat{W} x| \le \tau. \end{equation}
  The above is equal to the operator norm  $L\|\Delta\mat{W}\|_2$. Indeed, we have by  Cauchy-Schwarz and the definition of the operator norm that
  \[ |b^\top \Delta\mat{W} x| \leq  \|b\|_2 \|\Delta\mat{W} x\|_2 \leq  \|b\|_2 \|\Delta\mat{W} \| \| x\|_2  = L \|\Delta\mat{W} \|. \]
  Thus $L\|\Delta\mat{W}\|_2$ is an upper bound to~\eqref{eq:ernaoidnwlaiwnd}. Furthermore this upper bound is realized by simply choosing $x = \sqrt{L} u_1$ and $b = \sqrt{L} v_1,$ where $u_1$ and $v_1$ are the unit left and right singular vectors, respectively, associated to the largest singular value $d_1 = \|\Delta\mat{W}\|_2.$  Thus we have proven that~\eqref{eq:muon-alt} can be re-written as 
  \begin{equation}\label{eq:muon-alt-}
    \min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}} \quad \mbox{subject to} \quad
L \|\Delta\mat{W}\|_2 \leq \tau,
\end{equation}
to which the solution is
\begin{equation}
  \Delta\mat{W} = - \frac{\tau}{L} \mat{U} \mat{V}^\top.
\end{equation}
...
\[
\mathcal{G}(\mat{\Sigma})=\bigl\{\, bx^\top ~\big|~ b\in\R^{d_{\mathrm{out}}},\ x\in\R^{d_{\mathrm{in}}},\ \ip{\mat{\Sigma}^\dagger\vec(bx^\top),\vec(bx^\top)}\leq L\,\bigr\}.
\]
...
$\norm{u}_2^2\norm{v}_2^2\leq L$ and the objective becomes
\[
...
\[
\max_{\widetilde{\mat{G}}\in\mathcal{G}(\mat{\Sigma})}\ip{\widetilde{\mat{G}},\Delta\mat{W}}
=\max_{\norm{u}_2\norm{v}_2\leq\sqrt{L}}u^\top \mat{P}^{1/2}\Delta\mat{W} \mat{Q}^{1/2}v
...
%%%%%%%%%%%%%%%%%%%
\begin{proof}
By \Cref{lem:pseudogradient-spectral}, the support function of $\mathcal{G}_L(\mat{Q}\otimes \mat{P})$ is
\[
  \max_{\widehat{\mat{G}}\in\mathcal{G}_L(\mat{Q}\otimes \mat{P})}
  \ip{\widehat{\mat{G}},\Delta\mat{W}}
  =\sqrt L\,\norm{\mat{P}^{1/2}\Delta\mat{W} \mat{Q}^{1/2}}_2 .
\]
Since $\mathcal{G}_L(\mat{Q}\otimes \mat{P})$ is centrally symmetric, the constraint in~\eqref{eq:precond-muon-super} is therefore equivalent to
\[
  \sqrt L\,\norm{\mat{P}^{1/2}\Delta\mat{W} \mat{Q}^{1/2}}_2\le\tau,
\]
which gives~\eqref{eq:precond-muon-constraint}.
...
$\norm{\Delta\widetilde{\mat{W}}}_2\le\tau/\sqrt L$. By \Cref{lem:muon-lmo}, a least Frobenius-norm solution is
\[
  \Delta\widetilde{\mat{W}}
  =
  -\frac{\tau}{\sqrt L}\msign(\widetilde{\mat{G}}).
\]
...
\begin{cut}
\section{Gradient leverage scores} \label{asec:grad-leverage}


The reason the leverage scores are bounded in many settings is that they are invariant to linear transformations and rescaling of the function.

\begin{lemma}[Monotonicity of leverage scores under linear maps] \label{lem:leverage-linearmap}
Let $\phi(x)\in\mathbb{R}^r$ be a random feature vector and let
$\mat{A}\in\mathbb{R}^{d\times r}$. Define
\[
    g(x) := \mat{A}\phi(x),
    \qquad
    \mat{M} := \mathbb{E}[\phi(x)\phi(x)^\top],
    \qquad
    \mat{C} := \mathbb{E}[g(x)g(x)^\top] = \mat{A}\mat{M}\mat{A}^\top.
\]
Then, for every $x$ such that $\phi(x)\in\operatorname{range}(\mat{M})$,
\[
    g(x)^\top \mat{C}^\dagger g(x)
    \leq
    \phi(x)^\top \mat{M}^\dagger \phi(x).
\]
Moreover, if $\mat{A}$ is injective on $\operatorname{range}(\mat{M})$, then equality
holds:
\[
    g(x)^\top \mat{C}^\dagger g(x)
    =
...
Set
\[
    u := \mat{M}^{\dagger/2}\phi(x),
    \qquad
    \mat{B} := \mat{A}\mat{M}^{1/2}.
\]
Since $\phi(x)\in\operatorname{range}(\mat{M})$,
\[
    \phi(x)=\mat{M}^{1/2}u,
\]
and therefore
\[
    g(x)=Bu,
    \qquad
    \mat{C}=\mat{B}\mat{B}^\top.
\]
Hence
\[
    g(x)^\top \mat{C}^\dagger g(x)
    =
    u^\top \mat{B}^\top(\mat{B}\mat{B}^\top)^\dagger Bu.
\]
The matrix
\[
    \mat{P}:=\mat{B}^\top(\mat{B}\mat{B}^\top)^\dagger \mat{B}
\]
is the orthogonal projector onto $\operatorname{range}(\mat{B}^\top)$.
Consequently,
\[
    0\preceq \mat{P}\preceq \mat{I},
\]
and thus
\[
    g(x)^\top \mat{C}^\dagger g(x)
...
If $\mat{A}$ is injective on $\operatorname{range}(\mat{M})$, then
\[
    \operatorname{null}(\mat{A}\mat{M}^{1/2})
    =
    \operatorname{null}(\mat{M}^{1/2}),
\]
so
\[
    \operatorname{range}(\mat{B}^\top)
    =
    \operatorname{range}(\mat{M}).
\]
Since $u\in\operatorname{range}(\mat{M})$, we have $Pu=u$, and therefore the
...
This invariance is important, because it provides us with a way to bound the gradient leverage scores even when the gradients are unbounded in Frobenius norm.



 An example which has an unbounded gradients in the Euclidean norm, but a constant bounded gradient leverage scores.
\begin{lemma}[Leverage scores for squared loss on ellipsoidal data] \label{lem:leverage-bound-sphere}
Consider the loss function $f(\mat{W}) :=  \EE{x}{\ell(\mat{W}x)}$ where
\[
    \ell(z) = \frac{1}{2}\|z\|_2^2,
    \qquad
    x = \mat{A} u,
    \qquad
    u \sim \operatorname{Unif}(\mathbb{S}^{m-1}),
\]
where $\mat{A} \in \mathbb{R}^{m\times m}$ is nonsingular.

Since $\mat{W}$ is a linear least-squares problem, the gradients are unbounded in Frobenius norm. 
However, the gradient leverage scores are bounded by a constant 
\begin{eqnarray} \label{eq:leverage-bound-sphere}
 \| G(x) \|_{\Sigma} & \leq & \sqrt{\frac{m(m+1)}{2}}.
\end{eqnarray}
that depends only on the input dimension $m$ and not on $\mat{W}$ or the conditioning of $\mat{A}$.

\end{lemma}

\begin{proof}
   For the proof we will make the dependency on $\mat{W}$ explicit to avoid any confusion. For
$\mat{W} \in \mathbb{R}^{n\times m}$, define
\[
    \mat{G}_W(x) := \nabla_W \ell(\mat{W}x) = (\mat{W}x)x^\top,
    \qquad
    g_W(x) := \operatorname{vec}(\mat{G}_W(x)),
\]
and
\[
    \mat{\Sigma}(\mat{W}) := \mathbb{E}_x[g_W(x)g_W(x)^\top].
\]

Since $x=\mat{A}u$, we have by the chain-rule
\[
    g_W(x)
    =
    \operatorname{vec}\!\left(\mat{W}\mat{A}uu^\top \mat{A}^\top\right)
    =
    (\mat{A} \otimes \mat{W}\mat{A})(u\otimes u).
\]
Thus $g_W(x)$ is a linear image of the quadratic feature
\[
    \phi(u) := u\otimes u.
\]
By the monotonicity of leverage scores under linear maps (see~\Cref{lem:leverage-linearmap}) we have that
\begin{equation} \label{eq:eafezefzevf}
   \| \mat{G}_W(x)\|_{\mat{\Sigma}(\mat{W})}^2 = g_W(x)^\top \mat{\Sigma}(\mat{W})^\dagger g_W(x)
    \leq
    \phi(u)^\top
    \left(
        \mathbb{E}[\phi(u)\phi(u)^\top]
    \right)^\dagger \phi(u) =:s(u)
    .
\end{equation}
For $u$ uniform on $\mathbb{S}^{m-1}$, rotational invariance implies that
the leverage score $s(u)$ on the right-hand side is constant for ever $u$ on the sphere.
Because it is constant, we also have that 
\begin{equation} \label{eq:eafezefzevf2}
    s(u) = \E{s(u)}.
\end{equation}
 Let $\mat{M} := \mathbb{E}[\phi(u)\phi(u)^\top]$. Computing the expectation of the leverage score gives
\begin{equation} \label{eq:eafezefzevf3}
    \mathbb{E}[s(u)]
    =
    \mathbb{E}[\phi(u)^\top
  \mat{M}^\dagger
    ...
    \left(
        \mat{M} \mat{M}^\dagger
    \right) = \mathrm{rank}(\mat{M}).
\end{equation}
Where the last equation follows because the product of a matrix and its pseudoinverse is the orthogonal projector onto the range of the matrix. Thus its trace equals the rank of the matrix $\mat{M} .$
Now it remains to show taht $\mathrm{rank}(\mat{M}) = \frac{m(m+1)}{2}$.


Recalling that
\[
\phi(u)=u\otimes u=\operatorname{vec}(uu^\top), \quad \mbox{where } u\in\mathbb{S}^{m-1}.
\]
Since $uu^\top$ is symmetric for every $u$, we have
\[
\operatorname{range}(\mat{M})
\subseteq
\operatorname{span}\{\phi(u):u\in\mathbb{S}^{m-1}\}
\subseteq
\operatorname{vec}(\mathbb{S}^m),
\]
where $\mathbb{S}^m$ denotes the space of symmetric $m\times m$ matrices.

Conversely, the matrices $uu^\top$ span the space of symmetric matrices $\mathbb{S}^m$. Indeed, for each
$i$, by chooseing $u=e_i$ we have that
$
e_i e_i^\top
$
is a diagonal basis element, while for $i\neq j$,  by choosing $u=e_i+e_j$ we have that
$
(e_i+e_j)(e_i+e_j)^\top
-
e_i e_i^\top
...
e_j e_j^\top
=
e_i e_j^\top+e_j e_i^\top,
$
which gives the symmetric off-diagonal basis elements. Hence
\[
\operatorname{span}\{uu^\top:u\in\mathbb{S}^{m-1}\}
=
\mathbb{S}^m.
\]
...
\]
and thus
\begin{equation}  \label{eq:eafezefzevf4}
\operatorname{rank}(\mat{M})
\leq
\dim(\mathbb{S}^m)
=
m+\binom{m}{2}
...
Putting the steps~\eqref{eq:eafezefzevf},~\eqref{eq:eafezefzevf2},~\eqref{eq:eafezefzevf3} and~\eqref{eq:eafezefzevf4} 
together gives 
 $$ \| \mat{G}_W(x)\|_{\mat{\Sigma}(\mat{W})}^2 \leq s(u) \equiv \mathrm{rank}(\mat{M}) \leq \frac{m(m+1)}{2}$$
  for every $u$ on the sphere. Taking square roots gives the desired bound~\eqref{eq:leverage-bound-sphere}.

\end{proof}


The preceding example is not specific to the squared loss: analogous bounds can be obtained for broad classes of polynomial and power losses, including one-sided power losses such as squared ReLU, by expressing the stochastic gradients as linear images of suitable polynomial (or homogeneous) feature maps. More generally, uniform control of such leverage scores is closely related to bounds on Christoffel functions and coherence parameters, which have been extensively studied for polynomial and other structured feature spaces; see, e.g.,~\cite{CohenMigliorati2017,RauhutWard2012,PauwelsBachVert2018}.

Furthermore, the gradient leverage scores are alwaus bounded in expectation, as the following lemma shows.
\begin{lemma}[Holds in expectation]\label{lem:expectation}
For any distribution $\cD$ we have that $\displaystyle \EE{x}{\|\mat{G}(x)\|_{\mat{\Sigma}}^2} = \mathrm{rank}(\mat{\Sigma}) \le nm.$
\end{lemma}
\begin{proof}
...
\begin{align*}
 ...
 &= \EE{x}{\trace{\mat{\Sigma}^\dagger \vec(\mat{G}(x))\vec(\mat{G}(x))^\top}}  \\
 &= \trace{\mat{\Sigma}^\dagger \mat{\Sigma}} = \mathrm{rank}(\mat{\Sigma}),
 \end{align*}
which is at most $nm$ since $\mat{\Sigma} \in \R^{nm\times nm}$.
\end{proof}


As a simple example, if the set of inputs are finite, then the gradients scores are also bounded
\begin{lemma}[Finite support]\label{lem:finite-support}
Suppose $\support(\cD) = \{x_1,\dots,x_N\}$ is finite with $N$ elements, 
thus 
$$\mat{\Sigma} = \EE{x}{\vec(\mat{G}(x))\vec(\mat{G}(x))^\top} = \frac{1}{N}\sum_{i=1}^N \vec(\mat{G}_i)\vec(\mat{G}_i)^\top,$$
 where $\mat{G}_i := \mat{G}(x_i)$. It follows that~\eqref{eq:Lipschitz-local} holds with $L=N$.
\end{lemma}
\begin{proof}
Let $g_i = \vec(\mat{G}_i)$ and $\mat{M} := [g_1, \ldots, g_N],$ so $g_i = \mat{M} e_i.$ It follows that 
\[g_i^\top \mat{\Sigma}^{\dagger} g_i = N \,e_i^\top \mat{M}^\top (\mat{M} \mat{M}^\top)^{\dagger} \mat{M} e_i  \leq  N,\]
where we used that $ \mat{M}^\top (\mat{M} \mat{M}^\top)^{\dagger} \mat{M} $ is an orthogonal projector.
\end{proof}
\end{cut}
...
\end{equation}
We will use the fact that for any matrix $\mat{M}$ with full column rank and reduced SVD $\mat{M}=\mat{U}\mat{\Sigma} \mat{V}^\top$,
...
\end{equation*}
which is the $\mat{B}$-update solving the Product Muon LMO~\eqref{eq:decoupled-lora-lmo-app} with budget $\tau=\eta$. Analogous arguments hold for the $\mat{A}$-update.
...
\]
where $\mat{G}_Q$ is the gradient with respect to $\mat{W}_Q$, and symmetrically for $\mat{W}_K$. Setting $\mat{W}_Q=\mat{B}$ and $\mat{W}_K=\mat{A}^\top$ gives $\mat{C}_K=(\mat{A}\mat{A}^\top)^{1/2}$ and $\mat{G}_Q=\mat{G}_B$, and the pair of updates becomes the Product Muon step~\eqref{eq:product-muon-step} with $\eta=\epsilon$.
...
\label{app:matnormal}
In \Cref{sec:sample-loss-lmo} we treated the preconditioners $\mat{P}$ and $\mat{Q}$ as given when writing the update~\eqref{eq:polorafull-update}. Here we describe how we compute them using online updates from the factor gradients and a normalization that makes these updates invariant to the rescaling $(\mat{P},\mat{Q})\mapsto(aP,a^{-1}\mat{Q})$, arriving at the $p,q$ updates~\eqref{eq:ours-klcoupling} of \Cref{alg:ours}.
...
\]
with decay factor $\beta$. Just as we replace the factor gradients $\mat{G}_A$ and $\mat{G}_B$ in the objective of \Cref{sec:sample-loss-lmo} with averaged versions $\widehat{\mat{M}}_A$ and $\widehat{\mat{M}}_B$ (line~\ref{ln:mom} of \Cref{alg:ours}), we replace the sample-gradient second-moment matrix $\mat{\Sigma}$ (\Cref{lem:gradprop}) with the EMA of the outer products of the vectorized batch gradients,
\[
  \overline{\mat{\Sigma}}_t:=\EMA^{\beta_2}_{s\le t}[\vec(\mat{G}_s) \vec(\mat{G}_s)^\top],
\]
with $\beta_2\in [0, 1]$ the curvature decay in \Cref{alg:ours}.
...
\[
  (a^{-1}\mat{Q})\otimes(aP)=\mat{Q}\otimes \mat{P}
  \qquad\text{for every } a>0,
\]
so we want the optimizer to behave identically at $(\mat{P},\mat{Q})$ and
$(aP,a^{-1}\mat{Q})$. The update direction is the same at both, but
\[
  \hat q_t(aP_t)=a^{-1}\hat q_t(\mat{P}_t),
  \qquad
...
can underestimate $\norm{\mat{M}}_2$, and since the optimizer divides by this estimate, that would risk destabilizing training.  We therefore return the larger of the power-iteration value and the maximum row norm of $\mat{M}$, which does not depend on the cached start vector. We use $K_{\mathrm{pow}} = 8$ iterations.
...
\label{app:gauge}
The update size $\rho=\eta/(\norm{\mat{A}}_2+\norm{\mat{B}}_2)$~\eqref{eq:ours-rho} depends on the factorization of the merged product $\mat{B}\mat{A}$. A rescaling $(\mat{A},\mat{B})\mapsto(cA,c^{-1}\mat{B})$ with $c>0$ preserves $\mat{B}\mat{A}$, and hence the layer's output, while changing the denominator to
\[
