...
Let $x\sim \cD$ be the distribution of inputs to the layer, so that for an input $x\in \R^{d_{\mathrm{in}}}$ the output is $\mat{W}x\in \R^{d_{\mathrm{out}}}$. The distribution $\cD$ is induced by the distribution of the data and by the parameters of the preceding layers, but we omit this dependence for simplicity. Let
...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since we lack access to the distribution on inputs $\cD$. Instead we relax the constraint from holding for every $x \in \support(\cD)$ to holding with high probability under $\cD$, by defining an \emph{outer-gradient} set $\mathcal{G}_{\delta}$ such that
...
\end{equation}
holds with probability at least $1 - \delta$. Imposing that the right-hand side of \eqref{eq:outgrad1}
is less than $\tau$ is then a sufficient condition for the constraint in~\eqref{eq:sample-lmo} to hold for $x$ in a high-probability event. To guide our design of the outer-gradient set, we first make the structural observation that sample-wise gradients of linear layers are rank-one.
...
\label{eq:DeltaW-lin}
\end{equation}
% Linear layer depends on this merged update, so we constrain its linearization. 
...
\begin{equation}\label{e-grad-wrt-factors}
    ...
    \mat{G}_B := \nabla_B \mathcal{L}(\mat{W}_0+\mat{B}\mat{A}) = \mat{G} \mat{A}^\top.
\end{equation}
therefore, the linearized loss change is $\ip{\mat{G}_A,\Delta\mat{A}}+\ip{\mat{G}_B,\Delta\mat{B}}$.
...
\mbox{subject to} &
\norm{\mat{P}^{1/2}\mat{B}\Delta\mat{A} \mat{Q}^{1/2}}_2 \le \frac{\tau}{2},
\end{array}
&
\begin{array}{ll}
...
\mbox{subject to} &
\norm{\mat{P}^{1/2}\Delta\mat{B} \mat{A} \mat{Q}^{1/2}}_2 \le \frac{\tau}{2} .
\end{array}
\end{array}
...
\end{equation}
Since substituting~\eqref{eq:collapse-constraint} into~\eqref{eq:decoupled-direction-lmo} makes each factorwise LMO an instance of \Cref{lem:precond-muon}, the two minimizers are $\Delta\mat{A} = -\frac{\tau}{2} \mat{D}_A$ and $\Delta\mat{B} = -\frac{\tau}{2} \mat{D}_B$, where
\begin{equation}
  \begin{aligned}
  \mat{D}_A &=
  \mat{C}_B^{-1/2}\,
