...
The rest of the paper proceeds as follows. \rev{In \Cref{sec:sample-loss-lmo} we derive a steepest descent step whose constraint controls the change in the per-sample loss, and show that Muon is its isotropic special case.} In \Cref{sec:product-muon} we show how to make a Muon update product-aware for LoRA, resulting in Product Muon. 
...
\begin{revblock}
Every optimizer we compare against minimizes the same linearization $\ip{\mat{G},\Delta\mat{W}}$ of the loss and differs only in the set from which $\Delta\mat{W}$ is drawn. We fix that frame and ask which set is right for LoRA.
\end{revblock}
...
Muon is based on a spectral LMO~\cite{bernstein2025modular,pmlr-v38-carlson15,carlson2015preconditioned}, which minimizes the local linearization of the loss subject to a ball constraint,
\begin{equation}
 \begin{array}{ll}
...
and $\mat{G}=\mat{U}\mat{\Sigma} \mat{V}^\top$ is the reduced SVD (see~\Cref{lem:muon-lmo}). In
...
Let $x\sim \cD$ be the distribution of inputs to the layer, so that for an input $x\in \R^{d_{\mathrm{in}}}$ the output is $\mat{W}x\in \R^{d_{\mathrm{out}}}$. The distribution $\cD$ is induced by the distribution of the data and by the parameters of the \chg{preceeding}{preceding} layers, but we omit this dependence for simplicity. Let
...
However, this viewpoint is not entirely satisfying, since it does not directly relate to the loss function $\ell$ that we are trying to minimize. \rev{We therefore replace this trust region on the output by one on the loss.}
...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since \rev{backpropagation returns the batch gradient $\mat{G}$ and never the per-input gradients $\mat{G}(x)$}. So instead, we enforce it indirectly, by defining an outer-gradient set $\mathcal{G}$ that contains all possible per-input gradients $\mat{G}(x)$ (and negative gradients $-\mat{G}(x)$), so that
\begin{equation} \label{eq:outgrad1}
    \max_{x \in \support(\cD)}  |\ip{\mat{G}(x), \Delta\mat{W}}|
    \leq   \max_{\widetilde{\mat{G}} \in \mathcal{G}} \; \ip{\widetilde{\mat{G}}, \Delta\mat{W}}.
\end{equation}
...
\begin{revblock}
Any set containing the per-input gradients makes~\eqref{eq:outgrad1} valid, but a loose one buys a needlessly small step, so we make $\mathcal{G}$ as tight as what we know about $\mat{G}(x)$. Its shape is fixed by the chain rule.
\end{revblock}
\begin{restatable}[Gradients are rank one]{lemma}{gradprop}
  \label{lem:gradprop}
    For every $x \in \support(\cD)$ the per-input gradient factors as
...
\[ \mathcal{G}_L := \left \{ \widetilde{\mat{G}} = b x^\top ~\big|~ b\in\R^{d_{\mathrm{out}}},\ x\in\R^{d_{\mathrm{in}}},\ \|b\|_2\|x\|_2 \leq L \right \}. \]
Next we show that Muon can be seen as an LMO method, but where gradients are taken from the outer-gradient set.
\textbf{Lemma (Loss bounded View of Muon).} The closed form to the steepest descent method
$\min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}}$ subject to $|\ip{\hat{\mat{G}},\Delta\mat{W}}| \le \tau$ for every $\hat{\mat{G}} \in \mathcal{G}_L$
is given by the Muon update $\Delta\mat{W} = -\eta\,\mat{U}\mat{V}^\top$, where $\eta:=\tau/L$.
What this lemma tells us is that the Muon update guarantees that the linearized sensitivity constraint remains bounded. If either the step size $\eta$ or the Lipschitz constant is too big, then we have little control over the oscillations of the function. Yet a further issue is that this view relies on the assumption that the gradient is $L$ bounded, but this does not always hold in practice or may be effectively infinite. Our derivation of Muon relied on assuming the gradients are bounded in the standard Frobenius norm. Though this is a common assumption, it does not always hold, or it may give a loose bound.
\end{cut}
\begin{revblock}
Rank one fixes the \emph{shape} of $\mathcal{G}$; its \emph{size} still needs a scale. Bounding $\norm{\mat{G}(x)}_F$ by a constant is the obvious choice, but no such constant need exist, and when it does it is set by the single worst input, which leaves the constraint loose everywhere else. The scale we use instead is the one the gradients set themselves, their second moment
\end{revblock}
\begin{align}
    \mat{\Sigma} \;&:=\EE{x}{\vec(\mat{G}(x))\vec(\mat{G}(x))^\top} \in \R^{d_{\mathrm{out}} d_{\mathrm{in}} \times d_{\mathrm{out}} d_{\mathrm{in}}},
    \qquad
    \| \mat{G}(x) \|_{\mat{\Sigma}}^2
    := \vec(\mat{G}(x))^\top \mat{\Sigma}^\dagger \vec(\mat{G}(x)),  \label{eq:leverage-score}
\end{align}
\begin{revblock}
which is also the one quantity of this kind that an optimizer can accumulate online. We ask that the gradients be bounded in this metric,
\end{revblock}
\begin{equation} \label{eq:Lipschitz-local}
    \| \mat{G}(x) \|_{\mat{\Sigma}}^2 \leq L, \quad \mbox{for every } x \in \support(\cD),
\end{equation}
\rev{[Markov-inequality argument goes here: $\EE{x}{\|\mat{G}(x)\|_{\mat{\Sigma}}^2} = \mathrm{rank}(\mat{\Sigma})$ exactly, so $L = \mathrm{rank}(\mat{\Sigma})/\delta$ holds off a set of probability $\delta$.]}
which gives the outer-gradient set
\begin{equation}
...
\end{equation}
\rev{Since the LMO supplies only a direction and we set the magnitude separately in \Cref{sec:method}, the value of $L$, and hence of $\tau$, does not affect the resulting update.}

\paragraph{Kronecker approximation.}
Storing $\mat{\Sigma}$ is out of the question at $d_{\mathrm{out}} d_{\mathrm{in}}$ squared entries, let alone inverting it. Following~\cite{shampoo,kfac}, we approximate $\mat{\Sigma}$ by a Kronecker product of symmetric positive definite matrices $\mat{P}\in \R^{d_{\mathrm{out}} \times d_{\mathrm{out}}}$ and $\mat{Q} \in\R^{d_{\mathrm{in}} \times d_{\mathrm{in}}}$,
\begin{equation} \label{eq:pq-factor}
     \mat{\Sigma}  \approx \mat{Q} \otimes \mat{P},
\end{equation}
\rev{which keeps one scale per output coordinate and one per input coordinate, the two spaces the rank-one gradient of \Cref{lem:gradprop} already lives in.} We treat the \emph{preconditioners} $\mat{P}$ and $\mat{Q}$ as given for now, deferring their update rule to \Cref{app:matnormal}.
...
\begin{revblock}
Muon is the isotropic case of this step. Taking $\mat{\Sigma}=\mat{I}$ makes~\eqref{eq:outgrad-precon} the rank-one Frobenius ball $\{bx^\top : \norm{b}_2\norm{x}_2\le\sqrt L\}$, and $\mat{P}=\mat{Q}=\mat{I}$ in~\eqref{eq:polorafull-update} returns $\Delta\mat{W}=-(\tau/\sqrt L)\,\msign(\mat{G})$.
\begin{restatable}[Muon controls the per-sample loss]{corollary}{muonview}
  \label{lem:muon}
  At $\mat{\Sigma}=\mat{I}$ the solution of~\eqref{eq:precond-muon-super} is the Muon update $\Delta\mat{W}=-\eta\,\msign(\mat{G})$ with $\eta=\tau/\sqrt L$. Consequently a Muon step satisfies $|\ip{\mat{G}(x),\Delta\mat{W}}|\le\tau$ for every $x\in\support(\cD)$ whenever $\norm{\mat{G}(x)}_F^2\le L$.
\end{restatable}
So Muon controls the per-sample loss under an isotropic gradient metric, and \methodname{} differs from it by measuring that metric instead of assuming it.
\end{revblock}
...
\begin{revblock}
Muon follows from \Cref{lem:precond-muon} by setting the metric to the identity.
\muonview*
\begin{proof}
Apply \Cref{lem:precond-muon} with $\mat{P}=\mat{Q}=\mat{I}$. The constraint
in~\eqref{eq:precond-muon-constraint} becomes $\norm{\Delta\mat{W}}_2\le\tau/\sqrt L$
and~\eqref{eq:polorafull-update} becomes $\Delta\mat{W}=-(\tau/\sqrt L)\msign(\mat{G})$,
which is the Muon update with $\eta=\tau/\sqrt L$. For the second claim, at
$\mat{\Sigma}=\mat{I}$ the membership test in~\eqref{eq:outgrad-precon} reads
$\norm{\widetilde{\mat{G}}}_F^2\le L$, so $\norm{\mat{G}(x)}_F^2\le L$ places every
per-input gradient in $\mathcal{G}_L(\mat{I})$ and~\eqref{eq:outgrad1} gives
$|\ip{\mat{G}(x),\Delta\mat{W}}|\le\tau$.
\end{proof}
\end{revblock}
