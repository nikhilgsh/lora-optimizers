...
**State-producing mechanism runs must retain every planned analysis milestone.**
Before submission, list each offline measurement and the earliest optimizer
state it requires, then encode those exact steps with `--snapshot_steps` or an
appropriate checkpoint cadence. When the mechanism is unresolved, retain at
least one early/mid-training state at or before the first observed trajectory
divergence, one late state, and the final state. Never make the first retained
state later than a comparison the investigation may need: scalar JSONL
diagnostics cannot reconstruct `A`, `B`, `P`, `Q`, or `pair_state`. Verify the
resolved startup config records the requested state-saving schedule before
accepting the production launch.
