#!/bin/bash
# Parameterized full-polar (ns=8) damping sweep, r=256, opc — used for the
# eps_rel lr-robustness campaign on BOTH OLMo and Llama, and for both the
# ct-k1 (plain chord-tight) and clean-k2 (chord-tight-clean, picard=2) arms.
# Compares relative factor-Gram damping (precond_delta_relative) against the
# existing abs-damp / picard=2 baselines in the LOCAL lr basin (lr × {1/3,1,3})
# around each model's own full-polar optimum.
#
