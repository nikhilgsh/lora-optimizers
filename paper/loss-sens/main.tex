...
The rest of the paper proceeds as follows. In \Cref{sec:sample-loss-lmo} we derive a steepest descent step whose constraint controls the change in the per-sample loss. Approximating the gradient second moment by a Kronecker product gives a preconditioned Muon step, which recovers Muon in the isotropic case. In \Cref{sec:method} we make this step product-aware for LoRA. Product Muon is equivalent to this product-aware step with the preconditioners set to the identity. We then set both factor updates to have the same spectral norm so that the spectral norm of the merged update is bounded. All proofs are in \Cref{app:proofs}.
...
requires access to all possible input gradients. Instead we relax the constraint from holding for every $x \in \support(\cD)$ to holding with high probability, by defining an \emph{outergradient} set $\mathcal{G}_{\delta}$ such that
...
We now construct the outergradient set using the rank-one structure and second moment of the sample gradients. 
