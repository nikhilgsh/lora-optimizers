...
\begin{corollary}[Isotropic case]
  \label{lem:muon}
  If $\mat{\Sigma}\propto\mat{I}$ then the solution of~\eqref{eq:precond-muon-super} is $\Delta\mat{W} = -\tau\,\msign(\mat{G})$, which is the solution of the spectral LMO~\eqref{eq:lmo} with $\eta=\tau$.
\end{corollary}
\rev{Preconditioned Muon therefore spends the step budget across directions according to $\mat{\Sigma}$, where Muon spreads it uniformly.}
