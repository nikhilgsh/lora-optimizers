...
\end{equation}
We treat the \emph{preconditioners} $\mat{P}$ and $\mat{Q}$ as given for now, deferring to \Cref{app:matnormal} their update rule, which estimates $\mat{\Sigma}$ online using the approximation \eqref{eq:pq-factor} from batch gradients (\Cref{lem:batch-moment}).

Using the factorization $\mat{\Sigma}  \approx \mat{Q} \otimes \mat{P}$ in the outergradient set~\eqref{eq:outgrad-precon} and imposing the constraint in~\eqref{eq:sample-lmo} over $\widetilde{\mat{G}}$ in $\mathcal{G}_\delta(\mat{Q} \otimes \mat{P})$ gives the following preconditioned version of Muon (\Cref{lem:precond-muon}).
