...
\Cref{sec:weighted-estimation-01} then connects
...
across steps with the weights $m_{t,s}$ of~\eqref{eq:EMAweights-01}. Since the optimizer
...
\]
which \eqref{e-diag-kron-second-mom-model-01} approximates by
...
entry~\eqref{eq:ours-klcoupling-01}. It does not normalize each
...
\eqref{e-diag-kron-second-mom-model-01}, so normalizing the accumulated
...
\Cref{sec:weighted-lmo-01}. Normalizing the per-step estimates
