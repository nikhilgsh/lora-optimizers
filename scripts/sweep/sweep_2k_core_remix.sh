#!/bin/bash
# 2000-step sweep with --polar_core_remix_alpha as 6th positional. Tests the
# completed-core-metric prediction (α=1/4) plus a small bracket {0.125, 0.375}
# for sensitivity.
lr=${1:-3e-4}
...
python train_lora.py \
    ...
    --lora_r "$lora_r" \
    --lora_alpha "$lora_r" \
    --polar_core_remix_alpha "$polar_core_remix_alpha" \
