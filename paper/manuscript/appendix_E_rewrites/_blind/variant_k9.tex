...
\Cref{sec:weighted-lmo-k9} writes the weighted LMO and isolates the two
moments through which past steps enter the problem.
\Cref{sec:weighted-estimation-k9}
...
\Cref{sec:scale-placement-k9} explains why \Cref{alg:ours} normalizes the
...
\begin{equation}\label{eq:weighted-relaxed-k9}
 \begin{array}{ll}
...
\begin{equation}\label{eq:EMAweights-k9}
...
with the objective weights $m_{t,s}$ in~\eqref{eq:EMAweights-k9}. The
...
\label{sec:weighted-estimation-k9}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,topsep=2pt]
  ...
  \begin{equation}\label{e-diag-kron-second-mom-model-k9}
...
\]
which the diagonal Kronecker model~\eqref{e-diag-kron-second-mom-model-k9}
...
\eqref{eq:weighted-relaxed-k9} decomposes as
\[
...
\begin{align}
  ...
  \label{eq:observable-moments-k9}
\end{align}
where
...
\eqref{eq:observable-moments-k9}. In this paragraph, all quantities are
...
\begin{equation}\label{eq:factor-fit-k9}
  \hat{q}(\widehat P)
  =
  \argmin_{q>0}
...
\begin{lemma}[Consistency of the fit from factor gradients]\label{lem:consistency-k9}
...
fits~\eqref{eq:factor-fit-k9} return
\[
  \hat{q}(\widehat P)
  =
...
The weighted fit in~\eqref{eq:factor-fit-k9} would combine all past
...
\begin{equation}\label{eq:ours-klcoupling-k9}
\begin{aligned}
  \hat{q}_t(P_t)
...
\Cref{lem:consistency-k9} then predicts
\[
  Q_{t+1}
  =
  \diag\!\bigl(q_{t+1}/\norm{q_{t+1}}_\infty\bigr)
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt,topsep=2pt]
  ...
  divergence, correct in the limit. Here \Cref{lem:consistency-k9} is an
...
\label{sec:scale-placement-k9}

\methodname{} normalizes the accumulated vectors $q_{t+1},p_{t+1}$ when
forming $Q_{t+1},P_{t+1}$ in~\eqref{eq:ours-klcoupling-k9}. It does not
...
\eqref{e-diag-kron-second-mom-model-k9}. Normalizing the accumulated
...
\Cref{sec:weighted-lmo-k9}. Normalizing the estimates before
