#!/bin/bash
# Llama-3.2-1B × opc-sft-stage2 (all-4-configs, Llama-tokenized cache) × r=64
...
# value over plain polar (picard=1) on Llama? Clone of sweep_phase_L_1b_r64_k2.sh
# with model/data swapped to Llama and ns=8 + picard exposed as a knob.
# NOTE: chord-tight-clean uses the pre_norm='none' path (2026-05-28 fix), so
# ns=8 here is true full-whitening polar (no redundant Frob shrink).
#
