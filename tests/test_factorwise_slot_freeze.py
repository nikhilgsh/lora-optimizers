...
def test_freeze_holds_only_checkpointed_factorwise_slots():
    ...
    # `Q`/`P` are the input- and output-side diagonal metrics, renamed from
    # `D_in`/`D_out` (the rename is declared in
    # `CurvatureWhitenLoRA.PAIR_STATE_ALIASES`, which is what lets a
    # pre-rename checkpoint still resume).
    large_before = [
        (state["Q"].clone(), state["P"].clone())
        for state in optimizer.pair_state.values()
    ]
    moments_before = [
    ...
    for index, state in optimizer.pair_state.items():
        ...
        assert not torch.equal(state["Q"], large_before[index][0])
        assert not torch.equal(state["P"], large_before[index][1])
