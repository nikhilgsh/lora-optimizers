...
\section{A new loss sensitivity viewpoit on Muon}\label{sec:muon}
...
Here we show a new viewpoint for Muon. To describe this viewpoint, we first introduce some notation.
  Let $x\sim \cD$ be the distribution of inputs to the layer, thus for an input $x\in \R^{d_{\mathrm{in}}}$ the output is $\mat{W}x\in \R^{d_{\mathrm{out}}}$. The distribution $\cD$ is induced by the distribution of the data and by the parameters of the preceeding layers,  but we will omit this dependence for  simplicity.
Let 
...
Next we show that Muon can be seen as a LMO method such as~\eqref{eq:sample-lmo}, but where gradients are taken from the outer-gradient set.
...
What this lemma tells us is that  the Muon update guarantees that the linearized sensitivity constraint remains bounded. Specfically, if we use Muon with a learning rate $\eta$ and our gradients are $L$ bounded, then due to~\eqref{eq:outgrad1} we know that 
...
\end{proposition}
\begin{proof}
The feasible set is nonempty, closed, and convex, since it is an intersection of closed slabs and contains $\Delta\mat{W}=0$. For every feasible $\Delta\mat{W}$, by Jensen's we have that 
\[
