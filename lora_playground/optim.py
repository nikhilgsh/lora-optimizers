...
class CurvatureWhitenLoRA(Optimizer):
    ...
    ── CANONICAL SPEC: paper/manuscript/main.tex Algorithm 1 (`alg:ours`). ──
    ...
    def _cw_apply_per_pair(self, lr, cb, b1, eps):
        ...
        for i, (A, B) in enumerate(self.pairs):
            ...
            if self.kl_coupled:
                ...
                if self.precond == "factorwise" and not self.freeze_factorwise_slots:
...
def build_optimizer(
    ...
    # Delegate to the single declarative builder (lora_playground/optim_specs).
    # build_optimizer's kwargs mirror OptimizerConfig 1:1, so the config IS the
    # forwarding surface: a dropped flag now requires a visible `fixed=` in the
    # optimizer's spec, not a hand-edited branch. The legacy ~1150-line if/elif
    # was retired after the 77/77 attr+step equivalence proof
    # (tests/test_optim_factory_equivalence.py).
    from .optim_config import OptimizerConfig, CONFIG_FIELDS
    from . import optim_specs
    _loc = locals()
