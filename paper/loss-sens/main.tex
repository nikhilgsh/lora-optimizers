...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since we lack access to the distribution on inputs $\cD$. Instead we relax the constraint from holding for every $x \in \support(\cD)$ to holding with high probability, by defining an \emph{outer-gradient} set $\mathcal{G}_{\delta}$ such that
...
is less than $\tau$ is then a sufficient condition for the constraint in~\eqref{eq:sample-lmo}. To guide our design of the outer-gradient set, we first make the structural observation that sample-wise gradients of linear layers are rank-one.
