...
# + lora_init_b + precond_delta + precond_delta_relative. Allows a single
# disBatch tasks file to mix Init[A] / Init[AB], k=1 / k=3, and ε_rel damping
# settings.
#
...
#   7: lora_init_b (zero / gaussian / symmetric)
#   8: precond_delta (default 1e-6)
#   9: precond_delta_relative (0 / 1; default 0)
lr=${1:-3e-3}
...
lora_init_b=${7:-zero}
precond_delta=${8:-1e-6}
precond_delta_relative=${9:-0}
...
damp_args=(--precond_delta "$precond_delta")
