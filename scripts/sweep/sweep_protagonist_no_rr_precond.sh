#!/bin/bash
# E2 ABLATION wrapper: protagonist + --cw_no_rr_precond. Forces the r x r matrices
# C_B=B^T P B and C_A=A Q A^T to identity in the DIRECTION only, so the update is
# dA = msign(Mhat_A Q^-1/2) Q^-1/2 and dB = P^-1/2 msign(P^-1/2 Mhat_B). The diagonal
# metric (P,Q) and its KL estimator are untouched. Complement of sweep_protagonist_no_diag_curv.sh.
