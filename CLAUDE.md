...
- **No evocative experiment-setting labels in paper prose — name the actual config.** Banned: **"production setting"**, and the same family the /writing skill bans ("hero setting/run", "showcase", "the X arm", "production batch"). In the manuscript, say the concrete config instead — "at rank $r=256$ and our batch of $32768$ tokens", "Llama-3.2-1B on math at $r=256$" — not a framing noun standing in for it. (These shorthand labels are fine in internal `~/lora` notes and chat; the ban is for `paper/manuscript/` prose.)
