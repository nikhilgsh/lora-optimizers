...
to iMuon+radius (the coupling's $D_{in}$ EMA is accumulated but never read), bit-identical. So the
`−Shampoo` arm **reuses the existing run** and is *not* part of the rerun. Only `−radius` needs a
kl-diag rerun (the coupled $D_{in}$ stays active; only $\rho$ changes).
