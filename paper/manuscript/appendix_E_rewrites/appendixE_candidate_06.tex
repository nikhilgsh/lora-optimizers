...
curvature factors $P,Q$. \Cref{sec:weighted-estimation-06} derives these
quantities from LoRA factor gradients alone, and
\Cref{sec:scale-placement-06} explains why the accumulated vectors are
...
so that $\widehat M_t=\sum_{s\le t}m_{t,s}G_s$. For the curvature input it uses an
exponential moving average, which keeps
...
moments in~\eqref{eq:observable-moments-06}. In this paragraph all
...
\begin{lemma}[Invariance of the fixed fit]\label{lem:consistency-06}
...
$\norm{\diag P^\star}_\infty=\norm{\diag Q^\star}_\infty=1$, with
$P^\star,Q^\star$ positive-diagonal and $C^\star_A,C^\star_B\succ0$ (equivalently
$B$ of full column rank and $A$ of full row rank). For any diagonal $\widehat P\succ0$ and $\widehat Q\succ0$, the
...
closure $C_{A,t}=B_t^\top P_tB_t$ and $C_{B,t}=A_tQ_tA_t^\top$ available
then, inverted with the relative damping of \Cref{app:damping}, and folds it
into an EMA,
...
\end{aligned}
\end{equation}
Unrolling the EMA separates a geometric initialization tail from the weighted
fit,
\[
...
\]
Model each step's second moment as diagonal-Kronecker, so the $j$th block of the
step-$s$ moment is $\kappa_s q^\star_{s,j}C^\star_{A,s}$. Scoring $G_{A,s}$
against the damped inverse $K^A_s$ (\Cref{app:damping}) of the stored closure
$C_{A,s}=B_s^\top P_sB_s$, the same blockwise calculation as
\Cref{lem:consistency-06} gives a positive scalar multiple of the target,
\[
  \hat q_s=c_s\,\diag(Q^\star_s),
  \qquad
  c_s=\kappa_s\cdot\tfrac1r\Tr\!\big(K^A_s\,C^\star_{A,s}\big)>0 :
\]
the direction is $\diag(Q^\star_s)$ and the closure enters only through $c_s$.
Tail aside, $q_{t+1}=\sum_{s\le t}w_{t,s}c_s\diag(Q^\star_s)=S\diag(Q^\star_t)+e$
with $S=\sum_s w_{t,s}c_s>0$ and $\norm{e}_\infty\le S\,\delta^Q_t$,
\[
  \delta^Q_t=\sum_s\alpha_s\norm{\diag(Q^\star_s)-\diag(Q^\star_t)}_\infty
  \le\max_s\norm{\diag(Q^\star_s)-\diag(Q^\star_t)}_\infty,
  \qquad \alpha_s\propto w_{t,s}c_s ,
\]
the recency-weighted drift of the estimated factor. The $\infty$-norm
normalization is Lipschitz away from zero, so for $\delta^Q_t<1$
\[
  \Big\|\,q_{t+1}/\norm{q_{t+1}}_\infty-\diag(Q^\star_t)\,\Big\|_\infty
  =O(\delta^Q_t)+O(\beta_2^{\,t}) ,
\]
exactly $\diag(Q^\star_t)$ when $Q^\star$ is stationary over the window. No
assumption that $P_s$ equals $P^\star$ enters. Symmetrically,
...
average has the same shape as~\eqref{eq:ours-klcoupling-06}. The fit here
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt,topsep=2pt]
  ...
  objective, correct in the limit. Here \Cref{lem:consistency-06} is an
  ...
  estimate departs from consistency only through the drift of the estimated
  factor -- the closure enters only positive per-step scalars -- a deterministic
  weighted sum over the realized quantities, not a distributional limit.
\end{itemize}
...
largest entry when it forms $Q_{t+1},P_{t+1}$~\eqref{eq:ours-klcoupling-06},
...
model~\eqref{e-diag-kron-second-mom-model-06}. Normalizing the accumulated
...
\Cref{sec:weighted-estimation-06}. Normalizing the estimates before
