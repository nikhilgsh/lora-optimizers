...
The rest of the paper proceeds as follows. In \Cref{sec:sample-loss-lmo} we derive a steepest descent step whose constraint controls the change in the per-sample loss, and show that Muon is its isotropic special case. 
...
gives the \emph{Product Muon} step~\eqref{eq:product-muon-step}, see \Cref{app:memoryless}.
...
The update size $\rho$ in \eqref{eq:ours-rho} depends on factor scaling:
