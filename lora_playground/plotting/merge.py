...
def _select_run(runs: list[tuple[int, int, dict, list[dict]]]
                ) -> tuple[dict, list[dict]]:
    """Select one physical run without inferring a composite trajectory.

    ``runs`` contains same-key collisions as ``(final_step, priority, cfg,
    evals)``.  The most-progressed physical run wins; a tie uses group
    priority.  A producer-recorded ``resume_from`` string does not authorize
    concatenating records here.  Consumers that need a multi-attempt history
    must request an explicit, versioned lineage through the records API.
    """
    _final_step, _priority, cfg, evs = max(
        runs, key=lambda run: (run[0], -run[1])
    )
    return cfg, evs
...
def merge_runs(group_priority: Iterable[str],
    ...
    Dedup rule: **one physical run wins**. The run with the greatest final step
    is selected, with ties broken by group priority order (earlier group in
    ``group_priority`` wins). This compatibility helper never concatenates
    separate physical runs. Multi-attempt histories require an explicit,
    versioned lineage through the records API.
    ...
    return [_select_run(runs) for runs in collected.values()]
