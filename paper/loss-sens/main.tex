...
\rev{[TODO: lead-in from \Cref{sec:intro} --- Muon wins in pretraining but not on LoRA; the constraint is the design choice.]}
...
\rev{[TODO: a tighter $\mathcal{G}$ permits a larger step; the chain rule fixes its shape.]}
...
\end{cut}
\rev{[TODO: rank one fixes the shape of $\mathcal{G}$; the scale comes from the gradients' own second moment. Why this rather than a Frobenius ball: sphere around an anisotropic cloud versus the matched ellipsoid. Both admit a high-probability radius, so the argument is tightness, not existence.]}
\begin{align}
...
\end{align}
\rev{[TODO: $\mat{\Sigma}$ is estimable online. State the bound:]}
...
\begin{revblock}
\rev{[TODO: $\mat{\Sigma}=\mat{I}$ recovers Muon.]}
