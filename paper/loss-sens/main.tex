...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since we lack access to the distribution on inputs $\cD$. Instead we enforce it with high probability, by defining an \emph{outer-gradient} set $\mathcal{G}_{\delta}$ so that
...
\begin{align}
    \mat{\Sigma} \;&:=\EE{x}{\vec(\mat{G}(x))\vec(\mat{G}(x))^\top},~
    \| \mat{M} \|_{\mat{\Sigma}}^2
    := \vec(\mat{M})^\top \mat{\Sigma}^\dagger \vec(\mat{M}),  \label{eq:leverage-score}
\end{align}
and $\|\cdot\|_{\mat{\Sigma}}$ the associated Mahalanobis norm on $\mat{M}\in\R^{d_{\mathrm{out}} \times d_{\mathrm{in}}}$,
...
\end{equation}
gives a valid outer-gradient set (\Cref{lem:expectation}). \rev{Since the LMO supplies only a direction and the magnitude is set separately in \Cref{sec:method}, neither $\delta$ nor $\tau$ affects the resulting update.}
...
\end{equation}
after absorbing $\sqrt{\mathrm{rank}(\mat{\Sigma})/\delta}$ into $\tau$. To which a closed form is given by
...
\begin{restatable}[Muon controls the per-sample loss]{corollary}{muonview}
  \label{lem:muon}
  At $\mat{\Sigma}=\mat{I}$ the solution of~\eqref{eq:precond-muon-super} is the Muon update $\Delta\mat{W}=-\eta\,\msign(\mat{G})$ with $\eta=\tau$. Consequently a Muon step satisfies $|\ip{\mat{G}(x),\Delta\mat{W}}|\le\tau$ with probability at least $1-\delta$.
\end{restatable}
\end{revblock}
...
in~\eqref{eq:precond-muon-constraint} becomes $\norm{\Delta\mat{W}}_2\le\tau$
and~\eqref{eq:polorafull-update} becomes $\Delta\mat{W}=-\tau\,\msign(\mat{G})$,
which is the Muon update with $\eta=\tau$. For the second claim, at
$\mat{\Sigma}=\mat{I}$ the membership test in~\eqref{eq:outgrad-precon} reads
$\norm{\widetilde{\mat{G}}}_F^2\le d_{\mathrm{out}} d_{\mathrm{in}}/\delta$, which
by \Cref{lem:expectation} and Markov's inequality holds for $\mat{G}(x)$ with
probability at least $1-\delta$, so~\eqref{eq:outgrad1} gives
$|\ip{\mat{G}(x),\Delta\mat{W}}|\le\tau$ on that event.
\end{proof}
\end{revblock}
...
\]
and absorbing $\sqrt c$ into $\tau$ gives~\eqref{eq:precond-muon-constraint}.
