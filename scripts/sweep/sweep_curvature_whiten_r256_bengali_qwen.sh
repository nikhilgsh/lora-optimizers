#!/bin/bash
# Qwen2.5-1.5B × Aya-Bengali variant of sweep_curvature_whiten_r256_opc_qwen.sh —
# the SOAP-curvature / KL-Shampoo "better conditioning" family at r=256 on the
# Bengali SFT mixture, long-horizon (9000). Conditioning test on the new dataset
# axis: does a better-conditioned algo (kl-shampoo-polar) beat AdamW/chord-tight
# on Bengali? Matches the Qwen-Bengali adamw/chord cell config EXACTLY (model,
# Bengali Qwen-tokenized pre-packed seq2048 cache, bs4xga4, eval_every 250, r256,
# alpha=r) so eval_loss is directly comparable in the bengali leaderboard notebook.
#
