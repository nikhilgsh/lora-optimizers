#!/bin/bash
# Robustness pilot (cross-model): Llama-3.2-1B × opc-sft-stage2
# × seq=2048 × global_batch=16 × r=64 × packed_v1.1. NS-iteration (whitening) sweep variant.
#
# Clone of sweep_phase_L_1b_r64.sh; differences:
#   - --model_name meta-llama/Llama-3.2-1B (was allenai/OLMo-2-0425-1B)
#   - --data_dir reuses Phase L's opc cache (tokenizer-rebuild handled by
#     prepare_data_llama equivalent — see pre-flight note in the plan;
#     the OLMo-tokenized cache is NOT usable directly; this script
#     assumes data/opc_sft_stage2_all_packed_seq2048_llama32 has been
#     built first using --model_name meta-llama/Llama-3.2-1B).
#
...
#   4: seed
#   5: muon_ns_steps (NS iteration count = whitening-fraction knob)
lr=${1:-1e-2}
