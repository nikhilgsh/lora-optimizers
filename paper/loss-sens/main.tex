...
\min_{\Delta\mat{W}} \ip{\mat{G}, \Delta\mat{W}} \quad \mbox{subject to} \quad \sqrt{\mathrm{rank}(\mat{\Sigma})/\delta}\,\left\| \mat{P}^{1/2} \Delta\mat{W} \mat{Q}^{1/2} \right\|_2 \le \tau,
\end{equation}
whose minimizers are the positive multiples of
...
\begin{restatable}[Muon controls the per-sample loss]{corollary}{muonview}
  \label{lem:muon}
  At $\mat{\Sigma}=\mat{I}$ the minimizers of~\eqref{eq:precond-muon-super} are the positive multiples of $-\msign(\mat{G})$.
\end{restatable}
\end{revblock}
...
\end{equation}
Since substituting~\eqref{eq:collapse-constraint} into~\eqref{eq:decoupled-direction-lmo} makes each factorwise LMO an instance of \Cref{lem:precond-muon}, the minimizers are the positive multiples of $-\mat{D}_A$ and $-\mat{D}_B$, where
\begin{equation}
  \begin{aligned}
  \mat{D}_A &=
  \mat{C}_B^{-1/2}\,
...
\muonview*
\begin{proof}
Apply \Cref{lem:precond-muon} with $\mat{P}=\mat{Q}=\mat{I}$, for which
$\mat{P}^{-1/2}\msign(\mat{P}^{-1/2}\mat{G}\mat{Q}^{-1/2})\mat{Q}^{-1/2}=\msign(\mat{G})$.
\end{proof}
\end{revblock}
...
\]
which is~\eqref{eq:precond-muon-constraint}.
...
$\norm{\Delta\widetilde{\mat{W}}}_2\le\tau/\sqrt c$. By \Cref{lem:muon-lmo}, a least Frobenius-norm solution is
\[
  \Delta\widetilde{\mat{W}}
  =
  -\frac{\tau}{\sqrt c}\,\msign(\widetilde{\mat{G}}),
\]
a positive multiple of $-\msign(\widetilde{\mat{G}})$.
