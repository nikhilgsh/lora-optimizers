...
Since $\mat{\Sigma}$ contains $(d_{\mathrm{out}} d_{\mathrm{in}})^2$ entries, storing and inverting it is infeasible. Following~\cite{shampoo,kfac}, we approximate $\mat{\Sigma}$ by a Kronecker product of symmetric positive definite matrices $\mat{P}\in \R^{d_{\mathrm{out}} \times d_{\mathrm{out}}}$ and $\mat{Q} \in\R^{d_{\mathrm{in}} \times d_{\mathrm{in}}}$,
\begin{equation} \label{eq:pq-factor}
     \mat{\Sigma}  \approx \mat{Q} \otimes \mat{P}.
\end{equation}
We treat the \emph{preconditioners} $\mat{P}$ and $\mat{Q}$ as given for now, deferring to \Cref{app:matnormal} their update rule, which fits the approximation~\eqref{eq:pq-factor} online from batch gradients (\Cref{lem:batch-moment}).

Imposing the constraint in~\eqref{eq:sample-lmo} for every $\widetilde{\mat{G}}\in\mathcal{G}_\delta(\mat{Q} \otimes \mat{P})$ gives the following preconditioned version of Muon (\Cref{lem:precond-muon}).
...
\begin{corollary}[Isotropic case]
  \label{lem:muon}
  If $\mat{\Sigma}\propto\mat{I}$ then, up to rescaling $\tau$, the solution of~\eqref{eq:precond-muon-super} is 
  \[
  \Delta\mat{W} = -\tau\,\msign(\mat{G}),
  \]
  which is the solution of the spectral LMO~\eqref{eq:lmo} with $\eta=\tau$.
\end{corollary}
...
gives the \emph{Product Muon} step~\eqref{eq:product-muon-step}, which we discuss in \Cref{app:memoryless}.


\paragraph{Magnitude.}
Having derived the direction, we now set the magnitude by controlling the size of the factor updates in spectral norm. Treating $\mat{A}$ and $\mat{B}$ symmetrically, we use a single update size $\rho$ for both factors, and set $\norm{\Delta\mat{A}}_2=\norm{\Delta\mat{B}}_2=\rho$. Furthermore, we ensure that the linearized merged update \eqref{eq:DeltaW-lin} has spectral norm bounded by the learning rate $\eta$. By the triangle inequality and submultiplicativity,
\begin{equation}
\norm{\Delta\mat{W}}_2
\approx
...
To make the update~\eqref{eq:polora-update} practical for minibatch training, we integrate the following ingredients, resulting in the \methodname{} step summarized in \Cref{alg:ours}:
