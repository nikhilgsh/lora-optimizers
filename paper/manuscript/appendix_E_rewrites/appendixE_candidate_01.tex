...
the gradient it would have at the current weights $W_t$. Using samples from past steps
changes which samples enter the averages, not the point at which their
gradients are taken. The two resulting moments are
\[
\overline G_t
  =
...
used only for the direction. The weighted problem therefore relaxes to
\begin{equation}\label{eq:weighted-relaxed-01}
 \begin{array}{ll}
  ...
  \end{array}
\end{equation}
\Cref{alg:ours} never stores $\overline G_t$ or
$\overline\Sigma_t$ directly. The gradients of old samples at $W_t$ were
not formed, and the per-sample second moment is too large to keep.
...
$\widehat M_t$. The next subsection expresses both its needed projections
and the curvature EMA in terms of factor gradients.
...
\end{enumerate}
For the constraint moment, evaluating gradients at their observation steps
and collapsing the within-batch second moment reduce
$\overline\Sigma_t$ to
\[
...
\]
Thus the direction step only requires $\widehat M_tA_t^\top$ and
...
vectors therefore leaves the update unchanged.
