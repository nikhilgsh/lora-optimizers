#!/bin/bash
# NO-MAGNITUDE wrapper (curvature kept, magnitude rule removed): protagonist + --cw_unpinned
# + --lora_init_b symmetric. Identical to sweep_protagonist_double_ablation.sh except that
# --cw_no_diag_curv is dropped, so this is the curvature-only cell of the curvature x magnitude
# 2x2 ablation. The symmetric init is required for the same reason as in the double ablation:
# the unpinned step is unstable at B=0.
