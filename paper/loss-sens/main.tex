...
As mentioned in \Cref{sec:intro}, Muon has found success in pretraining, but has not been as immediately fruitful in LoRA finetuning. The Muon update is based on a spectral LMO~\cite{bernstein2025modular,pmlr-v38-carlson15,carlson2015preconditioned}, which minimizes the local linearization of the loss subject to a ball constraint,
\begin{equation}
 \begin{array}{ll}
...
|\ip{\mat{G}(x), \Delta\mat{W}}|
  ...
  \label{eq:sample-lmo}
\end{equation}
The above LMO is well-posed (\Cref{prop:wellposed}), but it cannot be easily solved, since we lack access to the distribution on inputs $\cD$. Instead we enforce it with high probability, by defining a centrally symmetric outer-gradient set $\mathcal{G}$ that contains a random sample gradient $\mat{G}(x)$, so that
...
is less than $\tau$ is a sufficient condition for the constraint in~\eqref{eq:sample-lmo} to hold for $x$ in a high-probability event. To guide our design of the outer-gradient set, we first make the structural observation that sample-wise gradients of linear layers are rank-one.
...
\end{cut}
Secondly, we can observe that if we define $\mat{\Sigma}$ to be the second-moment of the gradients
\begin{align}
...
\end{align}
and $\|\cdot\|_{\mat{\Sigma}}$ the associated Mahalanobis norm,
then $\EE{x}{\|\mat{G}(x)\|_{\mat{\Sigma}}^2} = \mathrm{rank}(\mat{\Sigma})$ and so by Markov's inequality, the set
\begin{equation}
...
\end{equation}
gives a valid outer-gradient set. \rev{Since the LMO supplies only a direction and we set the magnitude separately in \Cref{sec:method}, the value of $L$, and hence of $\tau$, does not affect the resulting update.}
...
$$\mat{\Sigma} = \EE{x}{\vec(\mat{G}(x))\vec(\mat{G}(x))^\top} = \frac{1}{N}\sum_{i=1}^N \vec(\mat{G}_i)\vec(\mat{G}_i)^\top,$$
 where $\mat{G}_i := \mat{G}(x_i)$. It follows that the local gradient bound holds with $L=N$.
\end{lemma}
\begin{proof}
