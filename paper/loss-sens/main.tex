...
requires access to all possible input gradients. Instead we relax the constraint from holding for every $x \in \support(\cD)$ to holding with high probability, by defining an \emph{outer-gradient} set $\mathcal{G}_{\delta}$ such that
...
By~\eqref{eq:outgrad1}, this bounds the first-order loss change~\eqref{eq:sample-lmo} with probability at least 
$1-\delta$.
...
% To guide our design of the outer-gradient set, we first make the structural observation that sample-wise gradients of linear layers are rank one.

We now construct the outer-gradient set using the rank one structure and second moment of the sample gradients. 
The chain rule gives the first property.
...
The rank one property restricts the structure of the gradients.
To bound their size with high probability, we use their second
...
=\mathrm{rank}(\mat{\Sigma})$.
By Markov's inequality, the following set contains $\mat{G}(x)$
...
Using Gram matrices, we rewrite the two constraint norms as
...
gives the \emph{Product Muon}~\eqref{eq:product-muon-step}, see \Cref{app:memoryless}.
...
The magnitude $\rho$ in \eqref{eq:ours-rho} depends on factor scaling:
among scalar rescalings that preserve the product, it is largest when
the factors have equal spectral norms (\Cref{app:gauge}).
...
\end{equation*}
which is the $\mat{B}$-update solving the Product Muon LMO~\eqref{eq:product-muon-step} with budget $\tau=\eta$. Analogous arguments hold for the $\mat{A}$-update.
