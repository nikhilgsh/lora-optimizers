...
outergradient constraint. The momenta $\widehat M_A,\widehat M_B$ supply
...
curvature factors $P,Q$. \Cref{sec:weighted-estimation-02} derives these
quantities from LoRA factor gradients alone, and
\Cref{sec:scale-placement-02} explains why the accumulated vectors are
...
\]
For the curvature input it uses an exponential moving average. We write the
induced weights on past steps as
\[
...
the same Kronecker model to $S_t$. Three approximations make the factors of
$S_t$ estimable from $G_{A,s}$ and $G_{B,s}$.
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,topsep=2pt]
  ...
  It is exact at batch size one. The accumulated second moment is then
  $\E_{s \leq t}[g_sg_s^\top]$.
  ...
  \end{equation}
  \item When relating moments of $G_{A,s}$ and $G_{B,s}$ to $S_t$, replace
...
update~\eqref{eq:DeltaW-lin}, with $\widehat M_t$ replacing the batch
gradient in~\eqref{eq:polorafull}, the objective decomposes as
\[
...
$Q^\star$ from factor gradients, we link their second moments to $S_t$.
Since
...
moments in~\eqref{eq:observable-moments-02}. In this paragraph all
...
average has the same shape as~\eqref{eq:ours-klcoupling-02}. The fit here
...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt,topsep=2pt]
  ...
  removes that scale. The EMA is exact over the per-step fits, and reading it
  as the weighted fit at the current matrices uses the slow-change
  approximation above.
\end{itemize}
...
forms $Q_{t+1},P_{t+1}$ in~\eqref{eq:ours-klcoupling-02}. It does not
...
model~\eqref{e-diag-kron-second-mom-model-02}. Normalizing the accumulated
...
\Cref{sec:weighted-estimation-02}. Normalizing the estimates before
