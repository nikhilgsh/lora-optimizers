#!/bin/bash
# Phase L (long-horizon 1B) at r=256 with SSC plumbing for chord-tight-clean
# — k=1 ablation. Identical to sweep_phase_L_1b_r256_ssc.sh except
# --picard_iters_override 1; at k=1 the anchored-FW cross-coupling correction
# is identically zero, so this launcher runs "polar + SSC clip" without any
# chord-tight cross-coupling (Lemma 4 / §A2 of docs/notes/polar_product/
# algorithm_ssc_kappa.md). Pairs with the k=2 ssc launcher for a Picard-
# correction ablation on the OPC-1B leaderboard.
# Per smoke at r=256 no-compile: peak ~42 GB / 2.44 s/step.
#
